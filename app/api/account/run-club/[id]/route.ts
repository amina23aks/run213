...
export async function DELETE(request:Request,{params}:{params:Promise<{id:string}>}){try{const customer=await requireCustomerRequest(request),{id}=await params;await removeOwnedRun(customer,id);revalidateTag("run-club","max");revalidatePath("/run-club");revalidatePath("/");return Response.json({ok:true},{headers:{"Cache-Control":"private, no-store"}})}catch(error){return fail(error)}}
