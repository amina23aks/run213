...
const guestIpLimiter = createDurableRateLimit({ prefix: "run213:checkout:guest-ip", limit: GUEST_IP_CHECKOUT_LIMIT, window: CHECKOUT_RATE_LIMIT_WINDOW });
const authenticatedIpLimiter = createDurableRateLimit({ prefix: "run213:checkout:auth-ip", limit: AUTHENTICATED_IP_CHECKOUT_LIMIT, window: CHECKOUT_RATE_LIMIT_WINDOW });
const phoneLimiter = createDurableRateLimit({ prefix: "run213:checkout:phone", limit: PHONE_CHECKOUT_LIMIT, window: CHECKOUT_RATE_LIMIT_WINDOW });
const uidLimiter = createDurableRateLimit({ prefix: "run213:checkout:uid", limit: AUTHENTICATED_UID_CHECKOUT_LIMIT, window: CHECKOUT_RATE_LIMIT_WINDOW });
