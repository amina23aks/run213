...
const rates = { Alger: { home: 450, desk: 350 }, Oran: { home: 600, desk: 450 } };
...
test("valid authenticated pending delivery edit recalculates money", () => { const result = edit(base, valid, { uid: "u1" }); assert.equal(result.totals.shippingDzd, 450); assert.equal(result.totals.totalDzd, 5450); });
...
test("Wilaya and mode changes use canonical shipping while merchandise and items stay unchanged", () => { const originalItems = structuredClone(base.items); const byWilaya = edit(base, { ...valid, wilaya: "Oran" }, { uid: "u1" }); const byMode = edit(base, { ...valid, deliveryMode: "desk" }, { uid: "u1" }); assert.deepEqual(byWilaya.items, originalItems); assert.equal(byWilaya.totals.itemsSubtotalDzd, 5000); assert.equal(byWilaya.totals.totalDzd, 5600); assert.equal(byMode.totals.shippingDzd, 350); });
