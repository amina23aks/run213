...
function shouldAnimateMarquee(entries, scrollWidth, clientWidth) { return dedupeEntries(entries).length > 1 && scrollWidth > clientWidth + 2; }
...
function selectIndexes(length, count, chooser = () => 0) { if (!Number.isInteger(length) || length <= 0) throw new Error("NO_ELIGIBLE_PARTICIPANTS"); if (![1,2,3].includes(count)) throw new Error("BAD_WINNER_COUNT"); if (count > length) throw new Error("WINNER_COUNT_EXCEEDS_ELIGIBLE"); const available = Array.from({ length }, (_, index) => index); const selected = []; while (selected.length < count) { const poolIndex = chooser(available.length, selected.length); selected.push(available[poolIndex]); available.splice(poolIndex, 1); } return selected; }
function publicWinner(id, data, monthKey, winnerSelectedAt, placement = 1) { if (!isEligible(data, monthKey)) return null; return { submissionId: id, publicName: data.publicName || data.name || "Runner", publicCaption: data.publicCaption || null, publicWilaya: data.publicWilaya || null, proofImage: { secureUrl: data.proofImage.secureUrl, width: data.proofImage.width, height: data.proofImage.height }, monthKey, winnerSelectedAt, placement }; }
function winnerIds(data = {}) { return Array.isArray(data.winnerSubmissionIds) && data.winnerSubmissionIds.length ? data.winnerSubmissionIds : data.winnerSubmissionId ? [data.winnerSubmissionId] : []; }
function legacyMonth(monthKey, data = {}) { const ids = winnerIds(data); return { monthKey, drawStatus: data.drawStatus === "drawn" || ids.length ? "drawn" : "not_drawn", winnerSubmissionId: ids[0] || null, winnerSubmissionIds: ids, winnerCount: ids.length }; }
function drawOnce(state, eligible, count = 1, chosenIndex = 0) { if (state.winnerSubmissionIds?.length) return state.winnerSubmissionIds; if (!eligible.length) throw new Error("NO_ELIGIBLE_PARTICIPANTS"); if (count > eligible.length) throw new Error("WINNER_COUNT_EXCEEDS_ELIGIBLE"); state.winnerSubmissionIds = selectIndexes(eligible.length, count, () => chosenIndex).map((index) => eligible[index].id); state.winnerSubmissionId = state.winnerSubmissionIds[0]; state.winnerCount = state.winnerSubmissionIds.length; return state.winnerSubmissionIds; }
...
test("no cloned marquee for one entry", () => assert.equal(shouldAnimateMarquee([{ id: "a", proofImage: { publicId: "p" } }], 999, 100), false));
test("no cloned marquee for low item counts that fit", () => assert.equal(shouldAnimateMarquee([{ id: "a", proofImage: { publicId: "p1" } }, { id: "b", proofImage: { publicId: "p2" } }], 200, 400), false));
test("marquee animates only on real overflow without cloning", () => assert.equal(shouldAnimateMarquee([{ id: "a", proofImage: { publicId: "p1" } }, { id: "b", proofImage: { publicId: "p2" } }], 500, 200), true));
...
test("zero eligible draw fails safely", () => assert.throws(() => selectIndexes(0, 1), /NO_ELIGIBLE_PARTICIPANTS/));
test("winnerCount 1, 2, and 3 returns that many unique indexes", () => { for (const count of [1, 2, 3]) assert.equal(new Set(selectIndexes(4, count, () => 0)).size, count); });
test("winnerCount exceeding eligible submissions fails", () => assert.throws(() => selectIndexes(2, 3), /WINNER_COUNT_EXCEEDS_ELIGIBLE/));
test("first draw saves requested winners", () => { const state = {}; assert.deepEqual(drawOnce(state, [{ id: "a" }, { id: "b" }], 1, 1), ["b"]); assert.equal(state.winnerSubmissionId, "b"); assert.equal(state.winnerCount, 1); });
test("repeated draw returns same saved winners", () => { const state = {}; assert.deepEqual(drawOnce(state, [{ id: "a" }, { id: "b" }], 1, 0), ["a"]); assert.deepEqual(drawOnce(state, [{ id: "a" }, { id: "b" }], 1, 1), ["a"]); });
test("simulated concurrent draw cannot replace saved winners", () => { const state = {}; const first = drawOnce(state, [{ id: "a" }, { id: "b" }], 1, 0); const second = drawOnce(state, [{ id: "a" }, { id: "b" }], 1, 1); assert.deepEqual(first, second); });
test("public winner serializer excludes private fields", () => assert.deepEqual(Object.keys(publicWinner("s1", { status: "approved", monthKey: "2026-07", publicName: "Amina", contact: "secret", instagram: "secret", proofImage: validImage }, "2026-07", "2026-07-01T00:00:00.000Z")).sort(), ["monthKey", "placement", "proofImage", "publicCaption", "publicName", "publicWilaya", "submissionId", "winnerSelectedAt"].sort()));
test("legacy month without draw fields loads as not drawn", () => assert.deepEqual(legacyMonth("2026-07"), { monthKey: "2026-07", drawStatus: "not_drawn", winnerSubmissionId: null, winnerSubmissionIds: [], winnerCount: 0 }));
