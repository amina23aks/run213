...
export function Header() {
  ...
  const { totalCount: favoriteCount, isHydrated: favoritesHydrated } = useFavorites();
  ...
  return (
    ...
    <header className="site-header">
      ...
      <div className="site-header__icons" aria-label="Header actions">
        <Link className="site-header__favorites" href="/favorites" aria-label="Favorites"><IconHeart />{favoritesHydrated && favoriteCount > 0 ? <span>{favoriteCount}</span> : null}</Link>
        <AccountMenu />
