...
function AdminRunClubWorkspace({ defaultMonth }: { defaultMonth: string }) {
  ...
  async function unlinkCustomer() {
    ...
    setLoading(true); setLinkError("");
    try {
      const response = await adminFetch(`/api/admin/run-club/submissions/${selected.id}/customer`, { method: "DELETE" });
      const result = await response.json();
      if (!response.ok) throw new Error(result.message || "Customer account could not be unlinked.");
      const updated = { ...selected, customerUserId: null };
      setSelected(updated); setItems(current => current.map(item => item.id === updated.id ? updated : item));
      setUnlinkOpen(false); setMessage("Customer account unlinked. The submission was preserved."); setMessageTone("success");
    } catch (error) { setLinkError(error instanceof Error ? error.message : "Customer account could not be unlinked."); }
    finally { setLoading(false); }
  }
