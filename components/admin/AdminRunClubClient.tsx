...
function AdminRunClubWorkspace({ defaultMonth }: { defaultMonth: string }) {
  ...
  async function appendWinner() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.message || "Additional winner failed.");
      setMessage(result.message || "Additional winner saved.");
      setConfirmAppendOpen(false);
      await load(false);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Additional winner failed.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      ...
      {confirmAppendOpen ? <div className="adminRunClubModalOverlay" role="presentation"><aside className="adminRunClubConfirm" role="dialog" aria-modal="true" aria-labelledby="append-confirm-title"><h2 id="append-confirm-title">Add another winner for {new Date(`${month}-01T00:00:00Z`).toLocaleDateString("en", { month: "long", year: "numeric", timeZone: "UTC" })}?</h2><p>One additional winner will be selected randomly from the remaining approved participants.</p><p>The existing winner will not be changed.</p><div className="adminRunClubActions"><button className="adminPrimary" disabled={loading} onClick={() => void appendWinner()} type="button">{loading ? "Selecting..." : "CONFIRM ADDITIONAL WINNER"}</button><button className="adminSecondary" disabled={loading} onClick={() => setConfirmAppendOpen(false)} type="button">CANCEL</button></div></aside></div> : null}
