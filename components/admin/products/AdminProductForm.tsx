  ...
  return (
    <form className="adminProductForm adminCard" onSubmit={onSubmit}>
      ...
      <AdminProductSection eyebrow="06" title="Colors and placement">
        ...
        <div className="adminProductGrid adminProductGrid--three"><ToggleCard label="DROP_001" checked={draft.showInDrop001} onChange={(checked) => onChange("showInDrop001", checked)} /><ToggleCard label="Featured Drop" checked={draft.showInFeaturedDrop} onChange={(checked) => onChange("showInFeaturedDrop", checked)} /><ToggleCard label="Shop The Look" checked={draft.showInShopTheLook} onChange={(checked) => onChange("showInShopTheLook", checked)} /></div>
        <div className="adminProductGrid adminProductGrid--three"><AdminProductField label="Sort"><input inputMode="numeric" value={draft.sortOrder} onChange={(event) => onChange("sortOrder", event.target.value)} /></AdminProductField><AdminProductField label="Featured sort"><input inputMode="numeric" value={draft.featuredSortOrder} onChange={(event) => onChange("featuredSortOrder", event.target.value)} /></AdminProductField><AdminProductField label="Look group"><input placeholder="summer-road" value={draft.lookGroupSlug} onChange={(event) => onChange("lookGroupSlug", event.target.value)} /></AdminProductField></div>
      </AdminProductSection>
