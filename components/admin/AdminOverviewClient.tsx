...
export function AdminOverviewClient() {
  ...
  return <AdminShell title="Overview" description="The signals that need action. Merchandise value always excludes delivery fees.">
    ...
    <article className="adminCard adminChartPanel">
      ...
      <small className="adminDataNote">{data?.chartTruncated ? "Financial totals, trend, and category detail are capped at 500 orders per period; order and status counts remain exact aggregates." : data?.returnEventsTruncated ? "Return costs are capped at 500 return events per period." : "Hover a point, or tap it on mobile, for all daily values."}</small>
    </article>
    ...
    <section><div className="adminSectionLabel"><p>FINANCIAL ADJUSTMENTS</p><span>Return costs and contribution · excludes other operating expenses</span></div><div className="adminOverviewGrid adminOverviewGrid--financialAdjustments">{financialAdjustmentCards.map((card) => <MetricCard key={card.key} card={card} value={data?.metrics[card.key]} unavailable={data?.unavailable.includes(card.key) ?? false} loading={loading && !data} />)}</div></section>
    ...
    <section className="adminCard adminAttention"><div className="adminCard__heading"><p>RECENT WISHLIST</p><h2>Latest Join the Club signups</h2><Link href="/admin/wishlist">VIEW ALL →</Link></div><div className="adminInsightRows">{data?.recentWishlist.length ? data.recentWishlist.map((subscriber) => <article className="adminSubscriberRow" key={subscriber.id}><span><strong>{subscriber.email}</strong><small>{subscriber.joinedAt ? new Intl.DateTimeFormat("en-GB", { dateStyle: "medium" }).format(new Date(subscriber.joinedAt)) : "Date unavailable"}</small></span></article>) : <p className="adminAttention__clear">No wishlist signups yet.</p>}</div></section>
