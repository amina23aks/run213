...
type Payload = { total: number; subscribers: Subscriber[]; nextOffset: number | null };
...
export function AdminWishlistClient() {
  ...
  const load = useCallback(async (offset = 0, append = false) => {
    ...
    try { const token = await getToken(); const params = new URLSearchParams({ offset: String(offset), search: query }); const response = await fetch(`/api/admin/wishlist?${params}`, { headers: { Authorization: `Bearer ${token}` } }); if (!response.ok) throw new Error("Subscriber list could not be loaded."); const next = await response.json() as Payload; setData((current) => append && current ? { ...next, subscribers: [...current.subscribers, ...next.subscribers] } : next); }
