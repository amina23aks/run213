...
export function AdminWishlistClient() {
  const [data, setData] = useState<Payload | null>(null); const [search, setSearch] = useState("");
  ...
  const load = useCallback(async (cursor: string | null = null, append = false, searchEmail = "") => {
    setLoading(true); setError("");
    try { const token = await getToken(); const params = new URLSearchParams(); if (cursor) params.set("cursor", cursor); if (searchEmail) params.set("search", searchEmail); const response = await fetch(`/api/admin/wishlist?${params}`, { cache: "no-store", headers: { Authorization: `Bearer ${token}` } }); if (!response.ok) throw new Error("Subscriber list could not be loaded."); const next = await response.json() as Payload; setData((current) => append && current ? { ...next, subscribers: appendUnique(current.subscribers, next.subscribers) } : next); }
  ...
  return <AdminShell title="Wishlist" eyebrow="AUDIENCE" description="Join the Club email subscribers, newest first.">
    ...
    <section className="adminCard adminInsightsPanel">
      <div className="adminInsightsToolbar"><strong>SUBSCRIBERS</strong><form onSubmit={(event) => { event.preventDefault(); void load(null, false, search.trim()); }}><input aria-label="Search subscriber email" placeholder="Exact email…" value={search} onChange={(event) => setSearch(event.target.value)} /><button type="submit">SEARCH</button></form></div>
      {error ? <div className="adminInsightState adminInsightState--error"><span>{error}</span><button onClick={() => void load()} type="button">TRY AGAIN</button></div> : loading && !data ? <p className="adminInsightState">Loading subscribers…</p> : !data?.subscribers.length ? <p className="adminInsightState">No subscribers match this view.</p> : <div className="adminInsightRows">{data.subscribers.map((subscriber) => <article className="adminSubscriberRow" key={subscriber.id}><span><strong>{subscriber.email}</strong><small>{subscriber.joinedAt ? new Intl.DateTimeFormat("en-GB", { dateStyle: "medium" }).format(new Date(subscriber.joinedAt)) : "Date unavailable"}{subscriber.status ? ` · ${subscriber.status}` : ""}</small></span><button onClick={() => setPending(subscriber)} type="button">REMOVE</button></article>)}</div>}
...
function appendUnique(current: Subscriber[], next: Subscriber[]) { const ids = new Set(current.map((item) => item.id)); return [...current, ...next.filter((item) => !ids.has(item.id))]; }
