...
const emptyDraft = { collectionId: "", collectionSlug: "", name: "", numberLabel: "", description: "", pricingMode: "regular" as "regular" | "sale", discountPercent: "0", priceDzd: "", compareAtPriceDzd: "", heroImageUrl: "", heroImagePublicId: "", figureImageUrl: "", figureImagePublicId: "", productIds: [] as string[], status: "draft", sortOrder: "", showAsHomepageFigure: false, homepageFigureOrder: "" };
...
export function AdminLooksClient() {
  ...
  const availableCollections = useMemo(() => collections.filter((collection) => collection.status !== "archived"), [collections]);
  ...
  const selectedProducts = useMemo(() => draft.productIds.flatMap((id) => { const product = products.find((item) => item.id === id); return product ? [product] : []; }), [draft.productIds, products]);
  const pricing = useMemo(() => calculateLookPricing({ mode: draft.pricingMode, enteredPriceDzd: Number(draft.priceDzd || 0), discountPercent: Number(draft.discountPercent || 0), products: selectedProducts }), [draft.discountPercent, draft.priceDzd, draft.pricingMode, selectedProducts]);
  ...
  async function save() {
    ...
    const body = JSON.stringify({ collectionId: draft.collectionId, collectionSlug, slug: slugPreview, name: draft.name, numberLabel: draft.numberLabel, description: draft.description, priceDzd: pricing.lookSellingPrice, compareAtPriceDzd: pricing.compareAtPriceDzd, discountPercent: pricing.discountPercent, isPromo: pricing.isPromo, costPriceDzd: pricing.totalLookCost, heroImage: { url: draft.heroImageUrl, alt: draft.name, publicId: draft.heroImagePublicId || undefined }, figureImage: draft.figureImageUrl ? { url: draft.figureImageUrl, alt: draft.name, publicId: draft.figureImagePublicId || undefined } : null, productIds: draft.productIds, status: draft.status, sortOrder: editingId ? toInteger(draft.sortOrder) ?? 1 : null, showAsHomepageFigure: draft.showAsHomepageFigure, homepageFigureOrder: editingId ? validation.homepageFigureOrder : null });
  ...
  function edit(look: Look) { const isSale = (look.discountPercent ?? 0) > 0 || look.isPromo === true; setEditingId(look.id); setDraft({ collectionId: look.collectionId, collectionSlug: look.collectionSlug, name: look.name, numberLabel: look.numberLabel ?? "", description: look.description, pricingMode: isSale ? "sale" : "regular", discountPercent: String(look.discountPercent ?? 0), priceDzd: String(look.priceDzd || ""), compareAtPriceDzd: look.compareAtPriceDzd ? String(look.compareAtPriceDzd) : "", heroImageUrl: look.heroImage.url, heroImagePublicId: look.heroImage.publicId ?? "", figureImageUrl: look.figureImage?.url ?? "", figureImagePublicId: look.figureImage?.publicId ?? "", productIds: look.productIds, status: look.status === "active" ? "active" : "draft", sortOrder: String(look.sortOrder), showAsHomepageFigure: look.showAsHomepageFigure, homepageFigureOrder: look.homepageFigureOrder === null ? "" : String(look.homepageFigureOrder) }); setErrors({}); window.scrollTo({ top: 0, behavior: "smooth" }); }
  ...
  return <AdminShell title="Looks" description="Build Shop The Look entries from real active products.">
    <div className="adminLookWorkspace">
      <section className="adminCard adminLookEditor"><div className="adminCard__heading"><p>LOOKS</p><h2>{editingId ? "Edit look" : "Create look"}</h2><span>{message}</span></div>
        ...
        <AdminLookSection number="01" title="Collection and name">{availableCollections.length ? null : <p className="adminEmptyState">Create a Look Collection first.</p>}<div className="adminProductGrid adminProductGrid--two"><AdminLookField label="Collection" error={errors.collectionId}><select disabled={!availableCollections.length} value={draft.collectionId} onChange={(event) => { const collection = collections.find((item) => item.id === event.target.value); patchDraft({ collectionId: event.target.value, collectionSlug: collection?.slug ?? "" }); }}><option value="">Choose collection</option>{availableCollections.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></AdminLookField><AdminLookField label="Look name" error={errors.name}><input placeholder="City Everyday" value={draft.name} onChange={(event) => patchDraft({ name: event.target.value })} /></AdminLookField></div><div className="adminProductGrid adminProductGrid--two"><AdminLookField label="Slug preview"><input value={slugPreview || "choose-collection-look-name"} readOnly /></AdminLookField><AdminLookField label="Number label"><input placeholder="LOOK 01" value={draft.numberLabel} onChange={(event) => patchDraft({ numberLabel: event.target.value })} /></AdminLookField></div><AdminLookField label="Description"><textarea rows={3} placeholder="Clean silhouettes designed to move with you." value={draft.description} onChange={(event) => patchDraft({ description: event.target.value })} /></AdminLookField><AdminLookField label="Pricing mode"><div className="adminPillGroup">{(["regular", "sale"] as const).map((mode) => <button className={draft.pricingMode === mode ? "isSelected" : undefined} key={mode} type="button" onClick={() => patchDraft({ pricingMode: mode, discountPercent: mode === "regular" ? "0" : draft.discountPercent === "0" ? "10" : draft.discountPercent })}>{mode === "sale" ? "On sale" : "Regular"}</button>)}</div></AdminLookField><div className="adminProductGrid adminProductGrid--two">{draft.pricingMode === "regular" ? <AdminLookField label="Look Selling Price" error={errors.priceDzd}><input inputMode="numeric" min="0" placeholder="5500" value={draft.priceDzd} onChange={(event) => patchDraft({ priceDzd: event.target.value, discountPercent: "0" })} /></AdminLookField> : <AdminLookField label="Discount %" error={errors.discountPercent}><input inputMode="numeric" min="1" max="99" placeholder="10" value={draft.discountPercent} onChange={(event) => patchDraft({ discountPercent: event.target.value })} /></AdminLookField>}</div>{selectedProducts.length ? <div className="adminPricingStats"><span>Sum of Product Selling Prices: {formatDzd(pricing.sumProductSellingPrices)}</span><span>Total Look Cost: {formatDzd(pricing.totalLookCost)}</span><span>Look Selling Price: {formatDzd(pricing.lookSellingPrice)}</span><span>Customer Saving: {formatDzd(pricing.customerSaving)}</span><span>Estimated Profit: {formatDzd(pricing.estimatedProfit)}</span><span>Profit Margin: {pricing.profitMarginPercent}%</span></div> : null}{pricing.missingCostProductNames.length ? <p className="adminProductHint">Some selected Products do not have a cost price.</p> : null}{pricing.belowCost ? <p className="adminProductHint">Warning: final Look price is lower than total Product cost.</p> : null}</AdminLookSection>
...
function validateDraft(draft: LookDraft): { ok: boolean; errors: FieldErrors; homepageFigureOrder: number | null } {
  ...
  if (draft.pricingMode === "regular" && (toInteger(draft.priceDzd) === null || (toInteger(draft.priceDzd) ?? 0) <= 0)) errors.priceDzd = "Look price must be greater than zero.";
  const discount = Number(draft.discountPercent || 0);
  if (draft.pricingMode === "sale" && (!Number.isFinite(discount) || discount <= 0 || discount >= 100)) errors.discountPercent = "Discount must be greater than 0 and less than 100.";
...
function formatApiError(raw: string) { try { const parsed = JSON.parse(raw) as { message?: string; error?: string; fieldErrors?: Record<string, string[]>; issues?: { fieldErrors?: Record<string, string[]> } }; const fieldErrors = parsed.fieldErrors ?? parsed.issues?.fieldErrors; const firstField = fieldErrors ? Object.values(fieldErrors)[0]?.[0] : null; return firstField ?? parsed.message ?? parsed.error ?? "Look could not be saved. Please try again."; } catch { return raw || "Look could not be saved. Please try again."; } }
