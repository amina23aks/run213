...
const emptyDraft = { collectionId: "", collectionSlug: "", name: "", numberLabel: "", description: "", heroImageUrl: "", heroImagePublicId: "", productIds: [] as string[], status: "draft", sortOrder: "1", showAsHomepageFigure: false, homepageFigureOrder: "" };
...
export function AdminLooksClient() {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      patchDraft({ heroImageUrl: data.secureUrl, heroImagePublicId: data.publicId ?? "" });
    } catch (error) { setErrors({ heroImageUrl: error instanceof Error ? error.message : "Upload failed." }); }
  ...
  async function save() {
    ...
    const body = JSON.stringify({ collectionId: draft.collectionId, collectionSlug, slug: slugPreview, name: draft.name, numberLabel: draft.numberLabel, description: draft.description, heroImage: { url: draft.heroImageUrl, alt: draft.name, publicId: draft.heroImagePublicId || undefined }, productIds: draft.productIds, status: draft.status, sortOrder: toInteger(draft.sortOrder) ?? 1, showAsHomepageFigure: draft.showAsHomepageFigure, homepageFigureOrder: validation.homepageFigureOrder });
  ...
  return <AdminShell title="Looks" description="Build Shop The Look entries from real active products.">
    <div className="adminLookWorkspace">
      <section className="adminCard adminLookEditor"><div className="adminCard__heading"><p>LOOKS</p><h2>{editingId ? "Edit look" : "Create look"}</h2><span>{message}</span></div>
        ...
        <AdminLookSection number="03" title="Hero / figure image"><p className="adminProductHint">This image is used for the Look page and for the homepage figure when enabled.</p><label className="adminUploadButton"><input type="file" accept="image/jpeg,image/png,image/webp" disabled={uploading} onChange={(event) => { const file = event.target.files?.[0]; event.target.value = ""; if (file) void upload(file); }} /><span>{uploading ? "Uploading…" : draft.heroImageUrl ? "Replace hero image" : "Upload hero image"}</span></label>{errors.heroImageUrl ? <small className="adminInlineError">{errors.heroImageUrl}</small> : null}{draft.heroImageUrl ? <figure className="adminLookPreview adminLookPreview--portrait"><Image src={draft.heroImageUrl} alt="Look hero preview" width={160} height={210} unoptimized /><button type="button" onClick={() => patchDraft({ heroImageUrl: "", heroImagePublicId: "" })}>Remove image</button></figure> : null}</AdminLookSection>
        <AdminLookSection number="04" title="Homepage figure"><label className="adminToggleCard"><input type="checkbox" checked={draft.showAsHomepageFigure} onChange={(event) => patchDraft({ showAsHomepageFigure: event.target.checked, homepageFigureOrder: event.target.checked ? draft.homepageFigureOrder : "" })} /><span>Show this Look in the homepage figure row</span></label><AdminLookField label="Figure order" helper="Lower numbers appear first." error={errors.homepageFigureOrder}><input inputMode="numeric" placeholder="1" disabled={!draft.showAsHomepageFigure} value={draft.homepageFigureOrder} onChange={(event) => patchDraft({ homepageFigureOrder: event.target.value })} /></AdminLookField></AdminLookSection>
        ...
        <AdminLookSection number="06" title="Order inside collection"><AdminLookField label="Look sort order" helper="Lower numbers appear first on the Collection page." error={errors.sortOrder}><input inputMode="numeric" value={draft.sortOrder} onChange={(event) => patchDraft({ sortOrder: event.target.value })} /></AdminLookField></AdminLookSection>
...
function validateDraft(draft: LookDraft): { ok: boolean; errors: FieldErrors; homepageFigureOrder: number | null } {
  ...
  const sortOrder = toInteger(draft.sortOrder);
  ...
  if (draft.showAsHomepageFigure) {
    ...
    if (homepageFigureOrder === null) errors.homepageFigureOrder = "Enter a valid homepage figure order.";
  }
