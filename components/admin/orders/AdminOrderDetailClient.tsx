...
export function AdminOrderDetailClient({ orderId }: { orderId: string }) {
  ...
  async function updateStatus(nextStatus: AdminOrderStatus, note?: string | null, returnCostDzd?: number) { setSaving(true); setMessage(null); try { const token = await getToken(); const response = await fetch(`/api/admin/orders/${orderId}/status`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ status: nextStatus, note, returnCostDzd }) }); const data = await response.json() as { order?: AdminOrderDetail; error?: string }; if (!response.ok || !data.order) throw new Error(data.error ?? "Status could not be updated."); setOrder(data.order); setMessage("Status updated."); } catch (error) { setMessage(error instanceof Error ? error.message : "Status could not be updated."); } finally { setSaving(false); } }
