...
export function AdminOrderDetailClient({ orderId }: { orderId: string }) {
  ...
  return <AdminShell title={order?.orderNumber ?? "Order"} description="Manage one order with secure server-side status transitions."><AdminAccessGate>
    ...
    {order ? <section className="adminOrderDetailWorkspace">
      ...
      <section className="adminOrderItems adminCard"><div className="adminCard__heading"><p>ITEMS</p><h2>Order items</h2></div>{order.items.map((item, index) => <article className="adminOrderItem" key={`${item.productId ?? index}-${index}`}>{item.image ? <Image src={item.image} alt={item.name} width={72} height={72} unoptimized /> : <span className="adminOrderItem__imageFallback">No image</span>}<div><strong>{item.name}</strong><small>{item.selectedSize ?? "No size"} · Qty {item.quantity}</small>{item.selectedColor ? <span className="adminColorSwatch"><i style={{ background: colorValue(item.selectedColor) }} aria-label={`Color ${item.selectedColor}`} /></span> : null}{item.lookName ? <small>Look: {item.lookName} · {item.lookPricingMode ?? "look"}</small> : null}</div><dl><div><dt>Unit</dt><dd>{formatDzdValue(item.unitPriceDzd)}</dd></div><div><dt>Total</dt><dd>{formatDzdValue(item.lineTotalDzd)}</dd></div><div><dt>Unit cost</dt><dd>{item.unitCostDzd == null ? "Cost unavailable" : formatDzdValue(item.unitCostDzd)}</dd></div><div><dt>Line cost</dt><dd>{item.lineCostDzd == null ? "Cost unavailable" : formatDzdValue(item.lineCostDzd)}</dd></div><div><dt>Profit</dt><dd>{item.estimatedLineProfitDzd == null ? "Cost unavailable" : formatDzdValue(item.estimatedLineProfitDzd)}</dd></div></dl></article>)}</section>
