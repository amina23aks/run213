  ...
  return <AdminShell title={order?.orderNumber ?? "Order"} description="Manage one order with secure server-side status transitions.">
    ...
    {order ? <section className="adminOrderDetailWorkspace">
      ...
      <section className="adminOrderCardsGrid">
        <AdminInfoCard title="Order summary" rows={[ ["Items subtotal", formatDzdValue(order.totals.itemsSubtotalDzd)], ["Shipping", formatDzdValue(order.totals.shippingDzd)], ["Total", formatDzdValue(order.totals.totalDzd)], ["Payment method", order.paymentMethod ?? "cash_on_delivery"], ["Payment status", order.paymentStatus ?? "Unknown"], ["Cost of goods", formatDzdValue(order.totals.costOfGoodsDzd)], ["Estimated profit before return costs", formatDzdValue(order.totals.estimatedProfitDzd)], ["Historical return cost", order.returnCostDzd == null ? "Not recorded" : formatDzdValue(order.returnCostDzd)], ...(order.returnCostDzd == null ? [] : [["Estimated contribution after return", formatDzdValue(estimatedContributionAfterReturn(order.totals.estimatedProfitDzd, order.returnCostDzd))] as [string, string]]) ]} />
      ...
      <section className="adminOrderItems adminCard"><div className="adminCard__heading"><p>ITEMS</p><h2>Order items</h2></div>{order.items.map((item, index) => { const isLookItem = Boolean(item.lookName); const contribution = getAdminItemContribution(item); return <article className="adminOrderItem" key={`${item.productId ?? index}-${index}`}>{item.image ? <Image src={item.image} alt={item.name} width={72} height={72} unoptimized /> : <span className="adminOrderItem__imageFallback">No image</span>}<div><strong>{item.name}</strong><small><span>{item.selectedSize ? `SIZE ${item.selectedSize}` : "Size not recorded"}</span>{item.selectedColor ? <span className="orderVariantColor"><i className="inlineSwatch" style={{ background: item.selectedColorHex ?? "transparent" }} aria-label={`Color ${item.selectedColor}`} title={item.selectedColor} />{item.selectedColor}</span> : <span>Color not recorded</span>}<span>Qty {item.quantity}</span></small>{item.lookName ? <small>Look: {item.lookName} · {item.lookPricingMode ?? "look"}</small> : null}</div><dl><div><dt>{isLookItem ? "Unit retail" : "Unit"}</dt><dd>{formatDzdValue(item.unitPriceDzd)}</dd></div><div><dt>{isLookItem && item.allocatedRevenueDzd != null ? "Allocated revenue" : "Total"}</dt><dd>{formatDzdValue(isLookItem && item.allocatedRevenueDzd != null ? item.allocatedRevenueDzd : item.lineTotalDzd)}</dd></div><div><dt>Unit cost</dt><dd>{item.unitCostDzd == null ? "Cost unavailable" : formatDzdValue(item.unitCostDzd)}</dd></div><div><dt>Line cost</dt><dd>{item.lineCostDzd == null ? "Cost unavailable" : formatDzdValue(item.lineCostDzd)}</dd></div><div><dt>{isLookItem ? "Est. line contribution" : "Profit"}</dt><dd>{contribution == null ? "Cost unavailable" : formatDzdValue(contribution)}</dd></div></dl></article>;})}</section>
...
function historyLabel(previous:string|null,status:string,note:string|null){const from=previous?`${title(previous)} → ${title(status)}`:title(status);return note?.startsWith("Admin correction")?`${from} · Admin correction`:from;}
function title(value:string){return value.charAt(0).toUpperCase()+value.slice(1);}
export function estimatedContributionAfterReturn(estimatedProfitDzd: number | null, returnCostDzd: number): number | null { return estimatedProfitDzd == null ? null : estimatedProfitDzd - returnCostDzd; }
export function getAdminItemContribution(item: AdminOrderDetail["items"][number]): number | null {
  if (item.lookName && item.allocatedRevenueDzd != null && item.lineCostDzd != null) return item.allocatedRevenueDzd - item.lineCostDzd;
  return item.estimatedLineProfitDzd;
}
