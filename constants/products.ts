...
function createMockProduct(input: MockProductInput): Product {
  return {
    ...
    images: [{ id: `${input.slug}-image-1`, url: input.image, alt: `${input.name} product image`, sortOrder: 0, isPrimary: true, colorId: null }],
    colors: input.colors.map(([name, hex], index) => ({ id: `${input.slug}-color-${index}`, name, hex })),
